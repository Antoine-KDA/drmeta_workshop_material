...
load("data/ipd_group_1.RData")
# load(url("http://www.stats4life.se/data/drm/ipd_group_1.RData"))
...
# linear trend
fiti_lin_ipd <- by(ipd, ipd$id, function(dat) glm(y ~ x + c, data = dat, family = binomial))
...
xref <- 25 # median(ipd$x)
newdata <- data.frame(dose = seq(20, 30, length.out = 100)) 
...
# load(url("http://www.stats4life.se/data/drm/ad_group_1.RData"))
ad
