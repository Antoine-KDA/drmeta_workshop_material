...
ggplot(pred_sq, aes(dose, pred)) +
  ...
  scale_y_continuous(transform = "log", breaks = c(.8, 1, 1.25, 1.5, 2))
...
fit_rcs_ad_fe <- dosresmeta(formula = e_b ~ rcs(dose, k), id = id, type = type, se = e_se,
                            cases = cases, n = PT, data = ad, proc = "2stage", method = "fixed")
...
ppredi_newlin <- ggplot(predi_lin, aes(dose, pred)) +
  ...
  scale_y_continuous(transform = "log", breaks = c(.65, 1, 1.5, 2, 3))
ppredi_newlin
...
fit_rcs_ad <- dosresmeta(formula = e_b ~ rcs(dose, k), id = id, type = type, se = e_se,
                         cases = cases, n = PT, data = ad, proc = "2stage")
summary(fit_rcs_ad)
...
ppredi_newrcs <- ggplot(predi_rcs, aes(dose, pred)) +
  ...
  scale_y_continuous(transform = "log", breaks = c(.5, 1, 2, 4, 8))
ppredi_newrcs
...
ggplot(pred_subg, aes(dose, pred)) +
  ...
  scale_y_continuous(transform = "log", breaks = c(.65, 1, 1.5, 2, 2.5))


# meta-regression
fit_mr <- dosresmeta(formula = e_b ~ rcs(dose, k), id = id, type = type, se = e_se,
                     cases = cases, n = PT, data = ad, proc = "2stage", mod = ~ z)
summary(fit_mr)
