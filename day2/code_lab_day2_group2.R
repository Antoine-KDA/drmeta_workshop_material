...
fit_sq_ad <- dosresmeta(formula = e_b ~ dose + I(dose^2), id = id, se = e_se, covariance = "md",
...
fit_rcs_ad_fe <- dosresmeta(formula = e_b ~ rcs(dose, k), id = id, se = e_se, covariance = "md",
                            sd = sd_y, n = n, data = ad, proc = "2stage", method = "fixed")
...
predi_lin <- data.frame(dose = newd) %>% 
  cbind(sapply(fit_lin_ad$bi, function(b) b * (newd$dose - newd$dose[1]))) %>% 
  pivot_longer(cols = -dose, values_to = "pred", names_to = "id")
# prediction for the average, and a new study
predi_newlin <- newd %>% 
  cbind(lapply(pred_bi_lin, function(x) x * (newd$dose - newd$dose[1])) %>% 
          bind_cols())
...
fit_rcs_ad <- dosresmeta(formula = e_b ~ rcs(dose, k), id = id, se = e_se, covariance = "md",
                         sd = sd_y, n = n, data = ad, proc = "2stage")
summary(fit_rcs_ad)
...
fiti_subg <- by(ad, ad$z, function(dat) {
  dosresmeta(formula = e_b ~ rcs(dose, k), id = id, se = e_se, covariance = "md",
             sd = sd_y, n = n, data = dat, proc = "2stage")
})

# prediction
pred_subg <- lapply(fiti_subg, predict, newdata = newd) %>% 
  bind_rows(.id = "z") %>% 
...
# meta-regression
fit_mr <- dosresmeta(formula = e_b ~ rcs(dose, k), id = id, se = e_se, covariance = "md",
                     sd = sd_y, n = n, data = ad, proc = "2stage", mod = ~ z)
summary(fit_mr)
...
pred_mr <- tapply(newd_mr, newd_mr$z, function(n) {
  cbind(n, predict(fit_mr, newdata = n))
}) %>% 
  bind_rows()
