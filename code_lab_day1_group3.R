...
load("data/ipd_group_3.RData")
# load(url("http://www.stats4life.se/data/drm/ipd_group_3.RData"))

# binary outcome
group_by(ipd, id) %>% 
  summarise(cases = sum(y),
            n = n(),
            PT = sum(time),
            rate = cases/PT)
...
# linear trend
fiti_lin_ipd <- by(ipd, ipd$id, function(dat) coxph(Surv(time, y) ~ x + c, data = dat))
...
# forest plot
forest(res_ipd, atransf = exp)

# estimated summary dose-response function
xref <- 3 # median(ipd$x)
newdata <- data.frame(dose = seq(0, 12, length.out = 100)) 
pred_long_ipd <- newdata %>% 
  mutate(mean_pred = exp(coef(res_ipd) * (dose - xref))) %>% 
  cbind(sapply(bi_rma_ipd$yi, function(b) exp(b * (newdata$dose - xref)))) %>% 
...
ppred_long_ipd <- ggplot(pred_long_ipd, aes(dose, pred, group = id)) +
  ...
  scale_y_continuous(transform = "log", breaks = c(.75, 1, 1.5, 2, 3))
ppred_long_ipd
...
# load data (locally or from url)
load("data/ad_group_3.RData")
# load(url("http://www.stats4life.se/data/drm/ad_group_3.RData"))
ad
...
fit_lin_ad <- dosresmeta(formula = e_b ~ dose, id = id, type = type, se = e_se,
                         cases = cases, n = PT, data = ad, proc = "2stage")
...
# forest plot
forest(res_ad, atransf = exp)

# estimated summary dose-response function
pred_long_ad <- newdata %>% 
  mutate(mean_pred = exp(coef(res_ad) * (dose - xref))) %>% 
  cbind(sapply(bi_rma_ad$yi, function(b) exp(b * (newdata$dose - xref)))) %>% 
  pivot_longer(cols = -c(dose, mean_pred), names_to = "id", values_to = "pred")

ppred_long_ad <- ggplot(pred_long_ad, aes(dose, pred, group = id)) +
  geom_line(col = "grey") +
  geom_line(aes(y = mean_pred), linewidth = 1.5) +
  scale_y_continuous(transform = "log", breaks = c(.5, 1, 2, 4))
ppred_long_ad


#' 4.	Comment main differences between the main results obtained from the 
#' individual and the aggregated data.

# comparison between mean estimates from ipd and ad
exp(cbind("ipd" = ci.lin(res_ipd)[, c("Estimate", "2.5%", "97.5%")], 
